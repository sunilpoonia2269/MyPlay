...
<%
    ...
    int val = Integer.parseInt(request.getParameter("getresult"));
    try {
        ...
        if (category.equals("All")) {
            ...
            ResultSet rs1 = st.executeQuery("select * from uploaded_videos LIMIT " + val + ", 10");
            while (rs1.next()) {
    ...
    <div class="random-class">
        <div class="resent-grid-img recommended-grid-img">
            <a href="single.jsp?vcode=<%=rs1.getString("vcode")%>">
                <video class="image-class" width="100%" poster="thumbnail/<%=rs1.getString("vcode")%>.jpg">
...
<%
    ...
    if (val == 0) {
        ResultSet rs1 = st.executeQuery("select * from uploaded_videos where category='" + category + "'");
        while (rs1.next()) {
            ResultSet getChannel1 = st1.executeQuery("select * from channel where chcode='" + rs1.getString("chcode") + "'");
            if (getChannel1.next()) {
